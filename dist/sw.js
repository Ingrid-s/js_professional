...
self.addEventListener('fetch', event => {
  const request = event.request; // aquí el único método que nos interesa es el get
...
async function cachedResponse(request) {
  ...
  const cache = await caches.open(VERSION); //aquí le preguntamos al cache si ya tiene la petición que querenoos hacer:

  const response = await cache.match(request); // aquí es importante agregar el || fetch(request), porque si no se tiene en cache, regresaría undefined y nunca ejecutaria la petición
